...
## ImageVolume vs Sidecar 目录共享对比

Kubernetes Pod 中共享目录有两种主流方式：**ImageVolume**（OCI 镜像卷）和 **Sidecar 容器**。本项目同时使用了这两种模式——ImageVolume 提供只读测试数据，rclone sidecar 提供 S3/NFS 挂载中转。

### 工作原理

```
ImageVolume (OCI 镜像作为卷):
  ┌──────────────────────────────────────┐
  │  Pod                                 │
  │  ┌──────────────────────────────┐    │
  │  │  fio-test 容器                │    │
...
Sidecar 共享目录:
  ...
  │  └────────────┘  └──────────────┘    │
  │        ↑                             │
  │  emptyDir 或 shared volumeMount      │
  │  容器间通过共享卷传递数据               │
  └──────────────────────────────────────┘
  特点: Sidecar 负责挂载/同步，主容器通过共享卷访问
```

### 对比总览

| 维度 | ImageVolume | Sidecar 共享目录 |
|------|-------------|-----------------|
| **数据来源** | OCI 镜像层（构建时预填充） | Sidecar 运行时挂载/同步 |
| **读写权限** | 🔒 只读 | 可读写（取决于挂载协议） |
| **额外容器** | 不需要 | 需要 1 个 sidecar 容器 |
| **资源开销** | 极低（bind mount，零 CPU） | 中（sidecar 消耗 CPU/内存） |
| **数据新鲜度** | 构建时固定，需重建镜像更新 | 实时/准实时（取决于同步策略） |
| **启动延迟** | Kubelet 拉取镜像层（可预缓存） | Sidecar 挂载就绪后才可用 |
| **K8s 版本要求** | ≥ 1.31 (Alpha, 需启用 feature gate) | 所有版本 |
| **镜像分发** | 通过容器镜像仓库（OCI 分发） | 不需要额外镜像 |
| **数据一致性** | 节点间完全一致（同一镜像 digest） | 取决于后端存储（S3/NFS 本身一致） |
| **动态数据** | ❌ 不支持 | ✅ 支持（写入、追加、修改） |
| **零拷贝** | ✅ Kubelet 直接 bind mount 镜像层 | ❌ 经过 FUSE/网络协议 + 共享卷 |
| **适合场景** | 模型权重、静态资源、只读配置 | 动态存储、缓存、实时数据同步 |

### 性能差异

| 指标 | ImageVolume | Sidecar (以 rclone S3 为例) |
|------|-------------|---------------------------|
| 读取路径 | 内核 bind mount → 本地磁盘 | 内核 FUSE → rclone(用户态) → HTTP S3 → 后端 |
| 协议转换次数 | 0 | 2 (FUSE → HTTP → S3) |
| 网络开销 | 无 (节点本地) | 有 (取决于后端位置) |
| 大文件吞吐 | 最高（~4 GB/s） | 较低（~1.2 GB/s） |
| 小文件 IOPS | 最高（~36k） | 较低（~17k） |

> 本项目中 ImageVolume 和 rclone sidecar 的实测数据参见上方[性能差异分析](#性能差异分析)。
...
需要写入数据？
  ├── 是 → Sidecar 共享目录
  │       ├── 需要 POSIX 语义 → NFS sidecar
  │       ├── 对象存储后端    → rclone/geesefs sidecar
  │       └── 跨平台兼容      → S3 FUSE sidecar
  │
  └── 否 → 数据是否频繁更新？
          ├── 是（分钟/小时级）→ Sidecar（实时同步）
          └── 否（天/周/版本级）→ ImageVolume
                  ├── 超大文件(GB级) → ImageVolume（零拷贝优势）
                  └── 需要跨节点一致性 → ImageVolume（OCI digest 保证）
```
...
| 组件 | 模式 | 目录 | 说明 |
|------|------|------|------|
| `fio-test` 容器 | ImageVolume | `/mnt/image` | 预置测试数据（256M/512M + 2000 小文件），只读 |
| `s3-sidecar` (rclone) | Sidecar | `/data/s3`, `/data/nfs` | rclone 挂载 S3 + NFS PVC，通过 emptyDir 共享给 fio-test |

**为什么 ImageVolume 不用 Sidecar 替代？** 因为测试数据是静态的、构建时就确定的——每次重建镜像即可更新数据。用 Sidecar 反而增加一个常驻容器的 CPU/内存开销，且无法享受零拷贝性能。**为什么 S3 不用 ImageVolume？** 因为 S3 数据是动态的——测试过程中需要写入、读取实时结果，ImageVolume 的只读特性无法满足。
